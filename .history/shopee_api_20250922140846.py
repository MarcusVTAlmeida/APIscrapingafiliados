...
def get_product_info_magalu(product_url):
    try:
        ...
        if "magazinevoce.com.br" in product_url:
            ...
            # Converte link normal em link afiliado (preserva query)
            path = re.sub(r"https?://www\.magazineluiza\.com\.br", "", product_url)
            # NOTE: aqui garantimos o underscore *entre* "magazine" e o id: "magazinein_603815"
            affiliate_link = f"https://www.magazinevoce.com.br/magazine{loja_corrigida}{path}"

        # Testa se o link funciona (opcional)
        response = requests.get(affiliate_link, timeout=10)
        if response.status_code != 200 or "Produto não encontrado" in response.text:
            return "Produto indisponível", "Preço indisponível", affiliate_link

        # Extrai o título do HTML (simples)
        ...
        return productname, price, affiliate_link

    except Exception as e:
        print("Erro Magalu:", e)
        # Retorna link mesmo se houver erro, para não quebrar o bot
        return "Produto Magalu", "Preço indisponível", product_url
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "shopee.com.br" in text:
        ...
        if affiliate_link:
            # uso de HTML para evitar que underscores sejam interpretados pelo Markdown
            msg = (
                f"📦 <b>{html_escape(productname)}</b>\n"
                f"💰 Preço: {html_escape(price)}\n"
                f"🔗 <a href=\"{affiliate_link}\">{html_escape(affiliate_link)}</a>"
            )
        else:
            msg = "❌ Não foi possível processar o link da Shopee."
    ...
    # Magalu (link normal OU link da loja Magazine Você)
        ...
        if affiliate_link:
            ...
            msg = "❌ Não foi possível processar o link do Magalu."
        await update.message.reply_text(msg, parse_mode='HTML', disable_web_page_preview=False)
