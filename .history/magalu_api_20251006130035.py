    ...
    try:
        ...
        if pix_panel:
            ...
            if match:
                ...
                # fallback: busca número + texto manualmente
                perc = re.search(r'(\d+\s*%)', discount_text)
                if perc:
                    if "pix" in discount_text.lower():
                        info["pix_discount"] = f"{perc.group(1)} de desconto No PIX"
                    else:
                        info["pix_discount"] = f"{perc.group(1)} de desconto"

            # Tenta extrair explicitamente "No PIX" se estiver separado
            if not info["pix_discount"]:
                pix_words = re.findall(r"pix", discount_text, re.I)
                if pix_words:
                    info["pix_discount"] = "Desconto No PIX"
