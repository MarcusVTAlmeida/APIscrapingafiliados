...
def format_price(price_raw):
    """Converte preço bruto da Shopee (em centavos x100) para R$"""
    if not price_raw:
        return None
    return float(price_raw) / 100000
...
def get_shopee_product_info(product_url):
    ...
    # Buscar informações do produto (agora também com preço antes do desconto)
    payload_product = {
        "query": f"""
            query {{
    ...
    productname, price, old_price, discount_percent, image_url = "Desconhecido", None, None, None, None

    nodes = info_data.get("data", {}).get("productOfferV2", {}).get("nodes", [])
    if nodes:
        node = nodes[0]
        productname = node.get("productName", "Desconhecido")
        min_price = format_price(node.get("priceMin"))
        max_price = format_price(node.get("priceMax"))
        old_price = format_price(node.get("priceBeforeDiscount"))
        image_url = node.get("imageUrl")

        # usa sempre o menor preço (geralmente o promocional)
        price = min_price
