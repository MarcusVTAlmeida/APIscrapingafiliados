...
# -------------------
# 🔗 TinyURL API OFICIAL (com token)
# -------------------
TINYURL_API_TOKEN = "3sclxt6fmgW3UE6k3EOnR70Ye9qWu0sDQRaMQ9b67ifufWaZVEab5SsFlV7Z"

def encurtar_link(url: str) -> str:
    """Encurta o link usando a API oficial do TinyURL."""
    try:
        ...
        if resp.status_code == 200 and "data" in data and "tiny_url" in data["data"]:
            return data["data"]["tiny_url"]
        else:
            print(f"[TinyURL] Erro ao encurtar: {data.get('errors', resp.text)}")
            return url  # fallback: retorna o link original se der erro

    except Exception as e:
        print(f"[TinyURL] Erro inesperado: {e}")
        return url  # fallback
...
def get_magalu_product_info(product_url):
    ...
    try:
        ...
        # 🔗 Encurta link automaticamente (via TinyURL API)
        ...
        if preco_cartao or parcelas:
            caption += f"\n💳 {preco_cartao or ''} {parcelas or ''}".strip()
