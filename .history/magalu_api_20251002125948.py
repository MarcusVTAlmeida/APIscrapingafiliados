...
def get_magalu_product_info(product_url):
    try:
        ...
        # -------------------
        # Monta o texto final em partes separadas
        # -------------------
        legenda = gerar_legenda()
        linhas = []
        linhas.append(f"{legenda} 😍")
        linhas.append(f"📦 {name}")
        if price_original:
            linhas.append(f"💰 De: {price_original}")
        if pix_text:
            linhas.append(f"👉 Por: {pix_text}")
        if card_text:
            linhas.append(f"💳 {card_text} no cartão")
        linhas.append(f"🔗 {affiliate_link}")

        # Junta tudo com quebras de linha
        final_text = "\n".join(linhas)

        return name, final_text, affiliate_link, image
