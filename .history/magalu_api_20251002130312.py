...
def montar_caption(name, price_original, pix_text, card_text, affiliate_link):
    parts = []
    if price_original:
    ...
    preco_compacto = " | ".join(parts) if parts else "Preço indisponível"

    legenda = gerar_legenda()
    caption = f"{legenda} 😍 📦 {name} 💰 {preco_compacto} 🔗 {affiliate_link}"
...
def get_magalu_product_info(product_url):
    try:
        ...
        # Monta caption compacto
        caption = montar_caption(name, price_original, pix_text, card_text, affiliate_link)
        return caption, image

    except Exception as e:
        print("Erro Magalu:", e)
        return "Produto Magalu indisponível", None
