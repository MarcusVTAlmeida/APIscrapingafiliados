...
def get_product_info_ml(product_url):
    try:
        ...
        # Nome (title da página ou og:title)
        name = None
        ...
        # Preço → Mercado Livre usa spans com classes andes-money-amount
    ...
    try:
        ...
        # Nome: Amazon geralmente usa <span id="productTitle">
        name_tag = soup.find("span", id="productTitle")
        name = name_tag.get_text(strip=True) if name_tag else None

        # Imagem: procurar meta og:image
        image_url = None
        tag = soup.find("meta", property="og:image")
        if tag:
            image_url = tag.get("content")

        # Preço: Amazon usa spans com classes como a-price-whole, a-offscreen etc.
        price = "Preço indisponível"
        # tentativa de preço integral
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Mercado Livre
        ...
        if affiliate_link:
            ...
            await update.message.reply_text("❌ Não foi possível processar o link da Amazon.")
