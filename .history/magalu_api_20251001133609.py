...
def get_magalu_product_info(product_url):
    try:
        ...
        if pix_panel:
            ...
            if price_pix:
                pix_text = f"{price_pix} \n{discount if discount else ''} {pix_method}".strip()
...
💰 De: {price_original if price_original else '---'} | Por: {pix_text if pix_text else '---'}
{f"💳 {card_text} no cartão" if card_text else ''}
