...
def get_product_info_magalu(product_url):
    try:
        ...
        # Nome do produto (via h1)
        match_title = re.search(
            r'<h1[^>]*data-testid="heading-product-title"[^>]*>(.*?)</h1>',
            html, re.DOTALL
        )
        productname = match_title.group(1).strip() if match_title else "Produto Magalu"

        # Preço do produto (via JSON)
        match_price = re.search(r'"price"\s*:\s*"([\d.,]+)"', html)
        if match_price:
            price_val = match_price.group(1).replace(".", "").replace(",", ".")
            price = f"R$ {price_val}"
        else:
        ...
        image_url = match_img.group(1) if match_img else None
