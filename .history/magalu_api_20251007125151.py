...
def get_magalu_product_info(product_url):
    ...
    try:
        ...
        # Busca adicional — mais abrangente
        if not info["price_original"]:
            # Caso tenha um texto explícito "De: R$"
            tag_riscado = soup.find(string=re.compile(r"De:\s*R\$"))
            if tag_riscado:
                info["price_original"] = tag_riscado.strip()

        # Busca final — qualquer <s> com valor em R$
        if not info["price_original"]:
            s_tags = soup.find_all("s")
            for s in s_tags:
                if re.search(r"R\$\s*\d+,\d{2}", s.get_text()):
                    info["price_original"] = s.get_text(strip=True)
                    break
