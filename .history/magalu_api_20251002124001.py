...
LEGENDAS = [
    ...
    "💯 Oferta válida por tempo limitado!! Aproveite essa oferta 😍",
...
def get_magalu_product_info(product_url):
    try:
        ...
        if pix_panel:
            ...
            pix_discount = discount_elem.strip() if discount_elem else None
            ...
            if price_pix:
                pix_text = f"{price_pix}"
                if pix_discount:
                    pix_text += f" | {pix_discount} {pix_method}"
        ...
        # -------------------
        # Monta o texto final (novo modelo)
        # -------------------
        legenda = gerar_legenda()

        caption_parts = []
        caption_parts.append(f"{legenda}")
        caption_parts.append(f"📦 {name}")

        if price_original or pix_text:
            preco_line = []
            if price_original:
                preco_line.append(f"De: {price_original}")
            if pix_text:
                preco_line.append(f"Por: {pix_text}")
            caption_parts.append("💰 " + " | ".join(preco_line))

        if card_text:
            caption_parts.append(f"💳 {card_text} no cartão")

        caption_parts.append(f"🔗 {affiliate_link}")

        caption = "\n".join(caption_parts)

        return name, caption, affiliate_link, image
