...
def get_magalu_product_info(product_url):
    try:
        ...
        # Desconto (ex: "10% de desconto no Pix")
        ...
        for p in soup.find_all("p"):
            if "R$" in p.get_text() and "Cartão" not in p.get_text():
                card_price_tag = p
                break
        price_card = card_price_tag.get_text(strip=True) if card_price_tag else None
