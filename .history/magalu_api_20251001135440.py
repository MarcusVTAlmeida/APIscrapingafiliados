...
def limitar_legenda(texto: str) -> str:
    """Trunca o texto para respeitar o limite de 1024 caracteres do Telegram."""
    if len(texto) > MAX_CAPTION_LENGTH:
        return texto[:MAX_CAPTION_LENGTH - 3] + "..."
    return texto
...
def get_magalu_product_info(product_url):
    try:
        ...
        # monta legenda final e limita
        caption = f"<b>{name}</b>\n{price_text}\n\n🔗 {affiliate_link}"
        caption = limitar_legenda(caption)
