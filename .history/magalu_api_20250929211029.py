...
def get_magalu_product_info(product_url):
    try:
        ...
        # Busca o preço à vista no cartão
        card_total_tag = soup.select_one("div[data-testid='mod-bestinstallment'] p.bTcHXB")
        if card_total_tag:
            card_total = card_total_tag.get_text(strip=True)

        # Busca a linha de parcelamento
        card_installment_tag = soup.select_one("div[data-testid='mod-bestinstallment'] p.bdQQwX")
        if card_installment_tag:
            card_installment = card_installment_tag.get_text(strip=True)
            if card_total:
                price_card = f"{card_total} ({card_installment})"
            else:
                price_card = card_installment
        elif card_total:
            price_card = card_total
