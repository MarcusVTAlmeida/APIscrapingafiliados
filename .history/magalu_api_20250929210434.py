...
def get_magalu_product_info(product_url):
    try:
        ...
        # Primeiro tenta pegar o valor total já pronto
        for p in soup.find_all("p"):
            txt = p.get_text()
            if "R$" in txt and "Cartão" not in txt and "Pix" not in txt and "ou " not in txt:
                price_card = txt.strip()
                break

        # Caso só tenha o formato de parcelas, calcula o total
        if not price_card:
