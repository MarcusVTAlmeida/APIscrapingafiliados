...
"""
        if price_original:
            texto_final += f"💰 De: {price_original} | "
        if pix_text:
            texto_final += f"Por: {pix_text}\n"
        if card_text:
            texto_final += f"💳 {card_text} no cartão\n"

        texto_final += f"\n🔗 {affiliate_link}\n"

        return name, texto_final.strip(), affiliate_link, image
