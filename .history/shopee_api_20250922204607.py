...
def get_product_info_magalu(product_url):
    try:
        # Corrige o nome da loja antes de gerar o link
        ...
        if "magazinevoce.com.br" in product_url:
            ...
            # Converte link normal em link afiliado (preserva path)
        ...
        response = requests.get(affiliate_link, timeout=10, headers={
        ...
        # 1️⃣ Tenta extrair JSON-LD com informações do produto
        match_jsonld = re.search(r'<script type="application/ld\+json">(.*?)</script>', html, re.DOTALL)
        productname, price, image_url = "Produto Magalu", "Preço indisponível", None

        if match_jsonld:
            try:
                data_json = json.loads(match_jsonld.group(1))
                if isinstance(data_json, list):
                    data_json = data_json[0]  # às vezes vem em lista

                productname = data_json.get("name", productname)
                price_val = data_json.get("offers", {}).get("price")
                if price_val:
                    price = f"R$ {price_val}"
                image_url = data_json.get("image", None)

            except Exception as e:
                print("Erro ao parsear JSON-LD:", e)

        # fallback caso não encontre JSON-LD
        if productname == "Produto Magalu":
            match_title = re.search(r'<title>(.*?)</title>', html, re.IGNORECASE)
            if match_title:
                productname = match_title.group(1).replace(" | Magazine Você", "").strip()
