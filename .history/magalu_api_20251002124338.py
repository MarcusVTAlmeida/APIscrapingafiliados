...
def get_magalu_product_info(product_url):
    try:
        ...
        # -------------------
        # Monta as partes separadas
        # -------------------
        legenda = gerar_legenda()
        legenda_linha = f"💯 {legenda} 😍"
        nome_linha = f"📦 {name}"

        preco_linha = ""
        if price_original:
            preco_linha += f"💰 De: {price_original}"
        if price_pix:
            if preco_linha:
                preco_linha += " | "
            pix_info = f"Por: {price_pix} {pix_method}"
            if discount:
                pix_info += f" com {discount}"
            preco_linha += pix_info
        if card_text:
            preco_linha += f"\n💳 {card_text} no cartão"

        link_linha = f"🔗 {affiliate_link}"

        # -------------------
        # Retorno no modelo desejado
        # -------------------
        price_text = f"{legenda_linha}\n{nome_linha}\n{preco_linha}\n{link_linha}"
