...
def _parse_price_to_float(price_text: str):
    """Converte preço 'R$ 1.254,44' para float"""
    if not price_text:
        return None
...
def _format_price(value: float):
    """Formata float 1254.44 → 'R$ 1.254,44'"""
    if value is None:
        return None
...
def get_magalu_product_info(product_url):
    try:
        ...
        # Se o site não mostrar o desconto no Pix, cria 10% automático
        if not product_info["pix_discount"]:
            pix_value = _parse_price_to_float(product_info["price_pix"])
            original_value = _parse_price_to_float(product_info["price_original"])
            if pix_value or original_value:
                # Se só tiver o original, calcula o pix com 10% a menos
                if not pix_value and original_value:
                    pix_value = original_value * 0.9
                    product_info["price_pix"] = _format_price(pix_value)
                product_info["pix_discount"] = "10% de desconto"
                product_info["pix_method"] = "no Pix"
