...
def get_magalu_product_info(product_url):
    try:
        ...
        # -------------------
        # Monta o texto final (versão curta)
        # -------------------
        parts = []
        if price_original:
            parts.append(f"💸 {price_original}")
        if pix_text:
            # tira o "com desconto" e "no Pix", deixando só o valor
            pix_clean = re.sub(r"\s*\(.*?\)", "", pix_text).strip()
            parts.append(f"💰 {pix_clean}")
        if card_text:
            # mostra só o valor total do cartão (sem parcelas longas)
            card_clean = card_text.split("(")[0].strip()
            parts.append(f"💳 {card_clean}")
