import re
import requests
from bs4 import BeautifulSoup

MAGALU_STORE = "in_603815"

def format_magalu_store(store_id: str) -> str:
    if not store_id.startswith("in_"):
        return store_id[:2] + "_" + store_id[2:]
    return store_id

def _sanitize(text):
    if not text:
        return None
    # remove NBSP e espaços extras
    return text.replace('\xa0', ' ').replace('\u202f', ' ').strip()

def _extract_discount_parts(discount_text):
    if not discount_text:
        return None, None
    # captura "10%" ou "10,5%" etc.
    m = re.search(r'(\d+(?:[.,]\d+)?%)', discount_text)
    percent = (m.group(1).replace(',', '.') + " OFF") if m else None
    # captura tudo após "OFF" (ex: "no PIX")
    m2 = re.search(r'OFF\s*(.+)', discount_text, re.I)
    method = m2.group(1).strip() if m2 else None
    return percent, method

def _parse_money(s):
    if not s:
        return None
    s = s.replace("R$", "").replace(".", "").replace(",", ".").strip()
    try:
        return float(s)
    except:
        return None

def _format_money(v):
    return f"R$ {v:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")

def get_magalu_product_info(product_url):
    try:
        loja_corrigida = format_magalu_store(MAGALU_STORE)

        # --- construir affiliate_link a partir da path (será retornado)
        if "magazinevoce.com.br" in product_url:
            affiliate_link = product_url
            # converter magazinevoce -> magazineluiza para scraping
            path = re.sub(r'https?://(www\.)?magazinevoce\.com\.br/magazine[^/]*', '', product_url)
            source_url = f"https://www.magazineluiza.com.br{path}"
        else:
            # se o user já passou a url do magazineluiza, usamos ela
            if "magazineluiza.com.br" in product_url:
                source_url = product_url
            else:
                source_url = product_url
            path = re.sub(r'https?://(www\.)?magazineluiza\.com\.br', '', source_url)
            affiliate_link = f"https://www.magazinevoce.com.br/magazine{loja_corrigida}{path}"

        # usa versão canônica sem query params para evitar variações de HTML
        source_url_clean = re.sub(r'\?.*$', '', source_url)

        headers = {
            "User-Agent": "Mozilla/5.0",
            "Accept-Language": "pt-BR,pt;q=0.9,en;q=0.8"
        }
        resp = requests.get(source_url_clean, headers=headers, timeout=10)
        soup = BeautifulSoup(resp.text, "html.parser")

        # Nome e imagem (og tags)
        tag = soup.find("meta", property="og:title")
        name = _sanitize(tag.get("content")) if tag and tag.get("content") else "Produto Magalu"

        tag = soup.find("meta", property="og:image")
        image = tag.get("content") if tag and tag.get("content") else None

        # preços e desconto (data-testid)
        price_current_tag = soup.find(attrs={"data-testid": "price-value"})
        price_current = _sanitize(price_current_tag.get_text()) if price_current_tag else None

        price_old_tag = soup.find(attrs={"data-testid": "price-original"})
        price_old = _sanitize(price_old_tag.get_text()) if price_old_tag else None

        discount_tag = soup.find(attrs={"data-testid": "tag"})
        discount_text = _sanitize(discount_tag.get_text()) if discount_tag else None

        discount_percent, discount_method = _extract_discount_parts(discount_text)

        # Se não houver price_old, tenta calcular a partir do desconto (quando possível)
        if not price_old and price_current and discount_percent:
            cur = _parse_money(price_current)
            if cur:
                # extrai número do discount_percent (ex: "10% OFF" -> 10.0)
                num = float(re.sub(r'[^\d,\.]', '', discount_percent).replace(',', '.'))
                if num < 100:
                    old = cur / (1 - num/100)
                    price_old = _format_money(old)

        # Monta texto final
        if price_current:
            price_text = f"💰 {price_current}"
            if price_old:
                price_text += f" (de {price_old})"
            if discount_percent and discount_method:
                price_text += f" - {discount_percent} {discount_method}"
        else:
            price_text = "Preço indisponível"

        return name, price_text, affiliate_link, image

    except Exception as e:
        print("Erro Magalu:", e)
        return "Produto Magalu", "Preço indisponível", product_url, None
